...
import User from '../client/client.model.js'
import { checkPassword, encrypt } from '../../utils/encrypt.js'
import { generateJwt } from '../../utils/jwt.js'
import userModel from '../../src/client/client.model.js'
...
export const registerUser = async(req, res)=>{
    try{
        ...
        return res.send({message: `Registered successfully, can be logged with username: ${user.username}`, success: true})
    }catch(error){
        console.error(error)
        return res.status(500).send({message: 'General error with registering user', error,  success: false})
    }
}
